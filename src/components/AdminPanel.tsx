...
export const AdminPanel: React.FC<AdminPanelProps> = ({ onLogoutAdmin }) => {
  ...
  return (
    <div className="w-full max-w-7xl mx-auto space-y-8 py-6 px-4 text-slate-800" id="admin_dashboard_element">
      ...
      {/* HEADER SECTION */}
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 bg-white p-4 sm:p-6 rounded-2xl border border-slate-200 shadow-sm mb-2 w-full overflow-hidden">
        <div className="flex items-center space-x-3 sm:space-x-4 text-left w-full lg:w-auto">
          <div className="p-2.5 sm:p-3 bg-purple-50 border border-purple-200 rounded-xl flex items-center justify-center shrink-0">
            <LayoutDashboard className="w-5 h-5 sm:w-6 sm:h-6 text-[#B5179E]" />
          </div>
          <div className="min-w-0 flex-1">
            <h2 className="font-display font-black text-xl sm:text-2xl lg:text-3xl text-[#56346F] tracking-tight truncate">
              M.A.P.A.™ Panel Administrativo
            </h2>
            <p className="text-[11px] sm:text-xs text-slate-500 font-sans tracking-wide truncate">
              Métricas exclusivas para el Administrador Principal (contacto@tupodermental.club)
            </p>
          </div>
        </div>

        <div className="grid grid-cols-1 xs:grid-cols-3 sm:flex sm:items-center gap-2 w-full lg:w-auto shrink-0">
          <button 
            onClick={handleGoogleAuthConnect}
            className="w-full sm:w-auto px-4 py-2.5 bg-indigo-50 hover:bg-indigo-100 border border-indigo-200 text-xs font-mono text-indigo-700 rounded-xl transition-all cursor-pointer font-bold flex items-center justify-center space-x-1.5"
          >
          ...
          <button 
            ...
            className="w-full sm:w-auto px-4 py-2.5 bg-slate-100 hover:bg-slate-200 border border-slate-200 text-xs font-mono text-slate-700 rounded-xl transition-all cursor-pointer font-bold text-center"
          >
            Actualizar
          </button>
          <button
            onClick={onLogoutAdmin}
            className="w-full sm:w-auto px-4 py-2.5 bg-rose-50 hover:bg-rose-100 border border-rose-200 text-xs font-mono text-rose-600 rounded-xl flex items-center justify-center space-x-1.5 transition-all cursor-pointer font-bold"
          >
      ...
      {isLoading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
            ...
            <div className="p-3.5 sm:p-5 rounded-2xl bg-white border border-slate-200 hover:border-slate-300 space-y-1.5 text-left shadow-sm hover:shadow-md transition-all duration-300">
              <div className="flex items-center justify-between text-slate-500">
                <span className="text-[9px] sm:text-[10px] font-mono font-bold tracking-wider uppercase truncate">REGISTRADOS</span>
                <Users className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-[#B5179E] shrink-0" />
              </div>
              <p className="text-2xl sm:text-4xl font-display font-black text-[#6E488A]">{metrics.totalUsers}</p>
              <span className="text-[9px] font-mono text-slate-500 leading-tight block truncate">Captura total acumulada</span>
            </div>

            {/* Cards 2: Active Users */}
            <div className="p-3.5 sm:p-5 rounded-2xl bg-white border border-slate-200 hover:border-slate-300 space-y-1.5 text-left shadow-sm hover:shadow-md transition-all duration-300">
              <div className="flex items-center justify-between text-slate-500">
                <span className="text-[9px] sm:text-[10px] font-mono font-bold tracking-wider uppercase truncate">ACTIVOS RECIENTES</span>
                <Clock className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-[#B5179E] shrink-0" />
              </div>
              <p className="text-2xl sm:text-4xl font-display font-black text-[#6E488A]">{metrics.activeUsers}</p>
              <span className="text-[9px] font-mono text-slate-500 leading-tight block truncate">Últimos 30 días de sintonía</span>
            </div>

            {/* Cards 3: Completed program */}
            <div className="p-3.5 sm:p-5 rounded-2xl bg-white border border-slate-200 hover:border-slate-300 space-y-1.5 text-left shadow-sm hover:shadow-md transition-all duration-300">
              <div className="flex items-center justify-between text-slate-500">
                <span className="text-[9px] sm:text-[10px] font-mono font-bold tracking-wider uppercase truncate">COMPLETADOS</span>
                <Award className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-[#B5179E] shrink-0" />
              </div>
              <p className="text-2xl sm:text-4xl font-display font-black text-[#6E488A]">{metrics.completedUsers}</p>
              <span className="text-[9px] font-mono text-slate-500 leading-tight block truncate">Avances de 7 Días completos</span>
            </div>

            {/* Cards 4: Rates completed */}
            <div className="p-3.5 sm:p-5 rounded-2xl bg-white border border-slate-200 hover:border-slate-300 space-y-1.5 text-left shadow-sm hover:shadow-md transition-all duration-300">
              <div className="flex items-center justify-between text-slate-500">
                <span className="text-[9px] sm:text-[10px] font-mono font-bold tracking-wider uppercase truncate">TASA FINALIZACIÓN</span>
                <TrendingUp className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-[#B5179E] shrink-0" />
              </div>
              <p className="text-2xl sm:text-4xl font-display font-black text-[#6E488A]">{metrics.completionRate}%</p>
              <span className="text-[9px] font-mono text-slate-500 leading-tight block truncate">Porcentaje de éxito total</span>
            </div>

            {/* Cards 5: Day Progress avg */}
            <div className="p-3.5 sm:p-5 rounded-2xl bg-white border border-slate-200 hover:border-slate-300 space-y-1.5 text-left shadow-sm hover:shadow-md transition-all duration-300 col-span-2 sm:col-span-1">
              <div className="flex items-center justify-between text-slate-500">
                <span className="text-[9px] sm:text-[10px] font-mono font-bold tracking-wider uppercase truncate">AVANCE PROMEDIO</span>
                <BarChart3 className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-[#B5179E] shrink-0" />
              </div>
              <p className="text-2xl sm:text-4xl font-display font-black text-[#6E488A]">{metrics.averageProgress} Días</p>
              <span className="text-[9px] font-mono text-slate-500 leading-tight block truncate">Mediana de sintonía activa</span>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
            ...
            <div className="lg:col-span-8 bg-white border border-slate-200 rounded-3xl p-4 sm:p-6 space-y-4 shadow-sm text-left overflow-hidden">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 border-b border-slate-100 pb-4">
                <div>
                  <h3 className="font-display font-black text-lg sm:text-xl text-[#56346F]">
                ...
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 w-full md:w-auto">
                  <div className="relative flex-1 md:w-60">
                    ...
                    <input 
                      ...
                      className="w-full bg-slate-50 border border-slate-200 focus:border-[#6E488A] p-2.5 pl-9 text-xs rounded-xl outline-none text-slate-800 font-sans transition-all placeholder:text-slate-400"
                    />
                  </div>
                  <button 
                    onClick={handleExportJSON}
                    disabled={usersList.length === 0}
                    className="p-2.5 bg-[#36C4D8] hover:bg-[#27A1B2] text-white font-mono font-black text-xs rounded-xl transition-all cursor-pointer disabled:opacity-40 select-none flex items-center justify-center space-x-1.5 px-4 shrink-0 border-none uppercase shadow-sm w-full sm:w-auto"
              ...
              {/* Users list: Mobile cards vs Desktop table */}
                  ...
                  <>
                    ...
                    <div className="block md:hidden space-y-3">
                      {filteredUsers.map((u, i) => {
                        const completedCount = u.completedDaysCount ?? 0;
                          ...
                          <div 
                            ...
                            className={`p-4 rounded-2xl border space-y-3 transition-all ${
                              u.disabled 
                                ...
                                : "bg-white border-slate-200 shadow-xs hover:border-purple-200"
                            }`}
                          >
                            {/* Header: Name + Email */}
                            <div className="space-y-0.5 text-left">
                              <div className="flex items-center justify-between gap-2">
                                <span className="font-extrabold text-sm text-slate-900 leading-tight">{u.nombre || "Sin Nombre"}</span>
                                <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[9px] font-mono uppercase font-black border shrink-0 ${
                                  isUserActive && !u.disabled
                                    ? "bg-emerald-50 text-emerald-700 border-emerald-200" 
                                    : "bg-slate-100 text-slate-500 border-slate-200"
                                }`}>
                                  <span className={`w-1.5 h-1.5 rounded-full ${isUserActive && !u.disabled ? "bg-emerald-500 animate-pulse" : "bg-slate-400"}`} />
                                  {isUserActive && !u.disabled ? "Activa" : "Inactiva"}
                                </span>
                              </div>
                              <p className="text-xs text-[#56346F] font-mono font-medium break-all">{u.email}</p>
                              {u.whatsapp && (
                                <p className="text-[10px] text-[#25D366] font-mono font-semibold pt-0.5">
                                  WhatsApp: {u.whatsapp}
                                </p>
                              )}
                            </div>

                            {/* Progress & Access Status Badge */}
                            <div className="flex items-center justify-between bg-purple-50/50 p-2.5 rounded-xl border border-purple-100/60 text-xs">
                              <div className="flex items-center gap-2">
                                <span className="px-2 py-0.5 rounded-full bg-[#EDE0F0] text-[#6E488A] font-extrabold font-mono text-[10px]">
                                  {completedCount} de 7 Días
                                </span>
                                <span className="text-[10px] text-slate-500 font-mono">
                                  Día actual: {u.currentDay || 1}
                                </span>
                              </div>

                              <span className={`px-2 py-0.5 rounded-full text-[9px] font-mono uppercase font-black border ${
                                u.disabled 
                                  ? "bg-rose-50 text-rose-700 border-rose-200" 
                                  : "bg-indigo-50 text-indigo-700 border-indigo-200"
                              }`}>
                                {u.disabled ? "Inhabilitada" : "Habilitada"}
                              </span>
                            </div>

                            {/* Full-width Toggle Action Button */}
                            <div className="pt-1">
                              <button
                                onClick={() => handleToggleUserStatus(u.email, !u.disabled)}
                                className={`w-full py-2.5 px-3 rounded-xl text-xs font-bold font-mono transition-all cursor-pointer border flex items-center justify-center gap-2 active:scale-98 ${
                                  u.disabled 
                                    ? "bg-emerald-50 hover:bg-emerald-100 text-emerald-700 border-emerald-200" 
                                    : "bg-rose-50 hover:bg-rose-100 text-rose-600 border-rose-200"
                                }`}
                              >
                                {u.disabled ? "HABILITAR USUARIA" : "INHABILITAR USUARIA"}
                              </button>
                            </div>
                          </div>
                        );
                      ...
                      <table className="w-full text-xs font-sans text-slate-700">
                        <thead>
                          <tr className="border-b border-slate-200 text-slate-600 uppercase font-mono tracking-wider text-[10px] text-left bg-slate-50">
                            ...
                            <th className="py-3 px-3 text-center">Avances</th>
                            <th className="py-3 px-3 text-center">Estado Activo</th>
                            <th className="py-3 px-3 text-right">Acceso / Acciones</th>
                          </tr>
                        </thead>
                        <tbody>
                          {filteredUsers.map((u, i) => {
                            ...
                            return (
                              ...
                              >
                                <td className="py-3.5 px-3 text-left max-w-[200px] xl:max-w-[280px]">
                                  <span className="font-bold text-slate-900 block leading-tight truncate">{u.nombre}</span>
                                  <span className="text-[10px] text-slate-500 block font-mono truncate">{u.email}</span>
                                  {u.whatsapp && (
                                    <span className="text-[9px] text-[#25D366] font-mono block font-semibold truncate">
                                      WhatsApp: {u.whatsapp}
                                    </span>
                                  )}
                                </td>
                                ...
                                <td className="py-3.5 px-3 text-center whitespace-nowrap">
                                  <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[9px] font-mono uppercase font-black border ${
                                    isUserActive && !u.disabled
                                      ? "bg-emerald-50 text-emerald-700 border-emerald-200" 
                                      : "bg-slate-100 text-slate-500 border-slate-200"
                                  }`}>
