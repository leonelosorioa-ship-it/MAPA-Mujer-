...
// ==========================================
// CENTRALIZACIÓN: COMPORTAMIENTO ESPEJO DE PROGRESO Y PREMIUM EN TIEMPO REAL
// ==========================================
app.get("/api/user/progress", authenticateJWT, (req, res) => {
  try {
    const { email } = req.query;
    if (!email) {
      return res.status(400).json({ error: "El correo electrónico es obligatorio para recuperar el progreso." });
    }

    const cleanEmail = String(email).toLowerCase().trim();
    if ((req as any).user.email !== cleanEmail) {
      return res.status(403).json({ error: "No autorizado para acceder a este perfil." });
    }

    const db = readUsersDB();
    const user = db.find((u: any) => u.email === cleanEmail);
    ...
    const userProgress = {
      ...
      hasDownloadedApp: !!user.hasDownloadedApp,
      unlockedAudios: user.unlockedAudios || [],
      initialScanResults: user.initialScanResults || null
    };
    ...
    return res.status(500).json({ error: "Error interno del servidor al recuperar el progreso." });
  }
});

app.post("/api/user/sync-progress", authenticateJWT, (req, res) => {
  try {
    const { email, programProgress, premiumData } = req.body;
    if (!email) {
      return res.status(400).json({ error: "El correo electrónico es obligatorio para la sincronización." });
    }
    ...
    // Try/catch for robust file-system locking and safe concurrent read/writes
    const db = readUsersDB();
    const userIndex = db.findIndex((u: any) => u.email === cleanEmail);

    if (userIndex === -1) {
      return res.status(404).json({ error: "Usuario no encontrado para sincronizar." });
    }
    ...
    // Update standard program progress fields with deep merging
    if (programProgress) {
      if (programProgress.currentDay !== undefined) db[userIndex].currentDay = Number(programProgress.currentDay);
      if (programProgress.completedDays !== undefined) {
        db[userIndex].completedDays = programProgress.completedDays;
        db[userIndex].isCompleted = programProgress.completedDays.length === 7;
      }
      if (programProgress.responses !== undefined) db[userIndex].responses = programProgress.responses;
      if (programProgress.completionTimestamps !== undefined) db[userIndex].completionTimestamps = programProgress.completionTimestamps;
      if (programProgress.dailyConclusionText !== undefined) db[userIndex].dailyConclusionText = programProgress.dailyConclusionText;
      if (programProgress.hasDownloadedApp !== undefined) db[userIndex].hasDownloadedApp = !!programProgress.hasDownloadedApp;
      if (programProgress.unlockedAudios !== undefined) db[userIndex].unlockedAudios = programProgress.unlockedAudios;
      if (programProgress.initialScanResults !== undefined) db[userIndex].initialScanResults = programProgress.initialScanResults;
    }

    // Update premiumData if provided
    if (premiumData) {
      db[userIndex].premiumData = {
        ...getOrCreatePremiumData(db[userIndex]),
        ...premiumData
      };
    }
    ...
    return res.json({
      success: true,
      message: "Estado espejo sincronizado de forma centralizada."
    });
  } catch (error) {
    console.error("Error en sync-progress endpoint:", error);
    return res.status(500).json({ error: "Error de servidor en sincronización distribuida." });
  }
});
